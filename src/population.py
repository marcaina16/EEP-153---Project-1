...
SEX = {
  "people": "Total", "total": "Total", "both": "Total",
...
def _bins(age_range: Tuple[int, int]) -> List[Tuple[int, int]]:
  lo, hi = age_range
  if lo > hi:
    raise ValueError("age_range must be (low, high) with low<=high")
  out = [(a, b) for (a, b) in BINS if not (hi < a or lo > b)]
  if not out:
    raise ValueError(f"age_range {age_range} doesn't overlap supported bins {BINS}")
  return out
